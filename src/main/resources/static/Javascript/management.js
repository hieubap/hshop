...
function changeHidden(id, type) {
    ...
    if (type == 0) {
        if (x.style.display === "none") {
            {
                document.getElementById("detail-bill-" + id).style.display = "inline-flex";
            }
        } else {
...
function showProducts() {
    ...
    <div><button class="btn btn-default btn-rm" onclick="deleteProduct(${product.id});" style="content:f2ed;margin-right:20px">
    <i class="fas fa-trash-alt"></i>
    </button>
    <button class="btn btn-default btn-ud" onclick="update(${product.id},1)" >
    <i class="fas fa-edit"></i>
    </button>
    <button class="btn btn-default btn-dt" onclick="changeHidden(${product.id},1)">
    ...
    document.getElementById('m2').innerHTML = 'Sản Phẩm<span class="notice">' + totalElements + '</span>';
    loadRender(false);
}

function showOrders() {
    var list = '<div>';
    for (i = (page_order * number_new_bill); i < orders.length && i < (page_order + 1) * number_new_bill; i++) {
        var arr = orders[i];
        ...
        <div style="width:25%;float: left">
            <div style="float: top;font-size: 17px"><b>tổng tiền</b>: ${convertPrice(arr.total+'')} </div>
            <div style="float: top;font-size: 17px"><b>trạng thái</b>:<div style="display:inline-block;width:10px;height:10px;border-radius:50%;background-color: ` + styleStatus + `"></div> ${arr.statusName} </div>
        </div>
        <div>
...
function showBills() {
    var list = '<div>';
    for (i = (page_bill * number_bill); i < bills.length && i < (page_bill + 1) * number_bill; i++) {
        var arr = bills[i];
...
function showNotifications() {
    var list = '<div>';
    for (i = (page_notification * number_notification); i < notifications.length && i < (page_notification + 1) * number_notification; i++) {
        ...
        list += `<div id="list-noti-${product.id}" class="list_product_order"` + isReadStyle + ` onclick="put(${product.id},4)">
...
function showStores() {
    var list = '<div>';
    for (i = page_store * number_store; i < stores.length && (page_store + 1) * number_store; i++) {
        var o = stores[i];
        list += `<div class="list_product_order">
...
function showTypeProducts() {
    var listType = '';
    var list = '<div>';
    for (i = (page_type_product * number_type_product); i < typeProducts.length && i < (page_type_product + 1) * number_type_product; i++) {
...
function showEvaluates() {
    var list = '<div>';
    for (i = (page_evaluate * number_evaluate); i < evaluates.length && i < (page_evaluate + 1) * number_evaluate; i++) {
        var o = evaluates[i];
        list += `<div class="list_product_order">
        <div style="width:25%;float: left;margin-left: 20px"><div style="float: top;font-size: 17px"><b>id</b>: ${o.id} </div>
        </div>
        ...
        if (response.status === 200) {
            ...
            response.json().then(function(text) {
                ...
                for (i = 0; i < (products.length / number_menu); i++) {
                    list += `<li><a href="#" onclick="changeTag(${i},1)">${i+1}</a></li>`;
                }
        ...
        if (response.status === 200) {
            ...
            response.json().then(function(text) {
                ...
                for (i = 0; i < (orders.length / number_new_bill); i++) {
                    listTag += `<li><a href="#" onclick="changeTag(${i},2)">${i+1}</a></li>`;
                }
                showOrders();
                document.getElementById('pageTag2').innerHTML = listTag;
                document.getElementById('m8').innerHTML = 'Đơn mới <span class="notice">' + body.totalElements + '</span>';
                loadRender(false);
            });
        }
    })
        ...
        if (response.status === 200) {
            ...
            response.json().then(function(text) {
                ...
                for (i = 0; i < (bills.length / number_bill); i++) {
                    list += `<li><a href="#" onclick="changeTag(${i},3)">${i+1}</a></li>`;
                }
                document.getElementById('pageTag3').innerHTML = list;

                showBills();
                document.getElementById('m3').innerHTML = 'Hóa đơn <span class="notice">' + body.totalElements + '</span>';
                loadRender(false);
            });
        }
    })
        ...
        if (response.status === 200) {
            ...
            response.json().then(function(text) {
                ...
                for (i = 0; i < (notifications.length / number_notification); i++) {
                    list += `<li><a href="#" onclick="changeTag(${i},4)">${i+1}</a></li>`;
                }
...
function countNotification() {
    fetch(url_notification_count_read, {
        ...
        if (response.status === 200) {
            response.json().then(function(text) {
                document.getElementById('m4').innerHTML = 'Thông báo <span class="notice">' + JSON.parse(JSON.stringify(text)).data + '</span>';
            })
        }
    });
}
    ...
    fetch(urlFetch, {
        ...
        if (response.status === 200) {
            ...
            response.json().then(function(text) {
                ...
                document.getElementById('m5').innerHTML = 'Cửa hàng<span class="notice">' + stores.totalElements + '</span>';
                stores = stores.data;

                showStores();
                let list = '';
                for (i = 0; i < (stores.length / number_store); i++) {
                    list += `<li><a href="#" onclick="changeTag(${i},5)">${i+1}</a></li>`;
                }
    ...
    fetch(urlFetch, {
        ...
        if (response.status === 200) {
            ...
            response.json().then(function(text) {
                ...
                document.getElementById('m6').innerHTML = 'Loại sản phẩm<span class="notice">' + typeProducts.totalElements + '</span>';
                typeProducts = typeProducts.data;

                let list = '';
                for (i = 0; i < (typeProducts.length / number_type_product); i++) {
                    list += `<li><a href="#" onclick="changeTag(${i},6)">${i+1}</a></li>`;
                }
    ...
    fetch(urlFetch, {
        ...
        if (response.status === 200) {
            ...
            response.json().then(function(text) {
                ...
                document.getElementById('m7').innerHTML = 'Đánh giá<span class="notice">' + evaluates.totalElements + '</span>';
                ...
                for (i = 0; i < (evaluates.length / number_evaluate); i++) {
                    list += `<li><a href="#" onclick="changeTag(${i},7)">${i+1}</a></li>`;
                }
...
function btnForm() {
    ...
    document.getElementById('ud-store-btn').addEventListener('click', function(e) {
        callAPIUpdateStore();
        let modalLayer = document.querySelector('.modal');
        modalLayer.style.display = 'none';
    });
...
function notificationNew() {
    document.getElementById('notification-new').addEventListener('click', function(e) {
