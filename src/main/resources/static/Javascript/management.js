    ...
    if (response.status === 200)
    {
      ...
      response.json().then(function (text){
        ...
        for (i = 0;i<arr.length&&i<number_bill;i++){
          list += '<div onclick="changeHidden('+arr[i].id+')" class="list_product_order">'
              + '<div style="float: left;margin-left: 20px;"><div style="float: top;font-size: 17px"><b>id</b>: '+arr[i].id+' </div>'
